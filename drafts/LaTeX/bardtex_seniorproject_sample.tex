...
\begin{figure}[H]
	...
	\\
	$(\mu_{s}=103.71, \sigma_{s}=71.99), (\mu_{n}=105.63, \sigma_{n}=74.95)$}
...
\begin{figure}[H]
	...
	\\
	$(\mu_{s}=1.79, \sigma_{s}=1.54), (\mu_{n}=2.16, \sigma_{n}=1.68)$}
	\label{fig:tweet_puncutation}
\end{figure}

\noindent
English stop words are ignored by the vectorizer because they are too common to help the classifier distinguish between the appropriate features for the classes. As shown in Figure \ref{fig:tweet_stop_words},our ground truth dataset contains tweets which are more likely to contain between 5 and 8 unique stop words than they are to contain fewer. In comparison Figure \ref{fig:tweet_unique_words} illustrates the distribution of the number \textbf{unique English words} per tweet in the ground truth dataset. As we expect of text-based posts within 280 characters in length, tweets containing fewer unique English words are more common.

\begin{figure}
	$$\includegraphics[width=\textwidth]{tweet_stop_words_histogram}$$
	\caption{Histogram of Ground Truth Unique English Stop Words
	\\
	$(\mu_{s}=7.14, \sigma_{s}=1.31), (\mu_{n}=7.05, \sigma_{n}=1.39)$
	}
...
\begin{figure}
	...
	\\
	$(\mu_{s}=16.80, \sigma_{s}=10.95), (\mu_{n}=16.25, \sigma_{n}=11.15)$
	}
...
\noindent
The average $F_{1}$ scores for non-subtweets and subtweets are respectively 71.66\% and 73.05\%. Thus, our classifier performs similarly well on both.

\section{Known Subtweeters}
\label{known_subtweeters_analysis}

In order to test how the classifier performed on tweets from known subtweeters, we acquired all publicly available tweets from 11 different accounts from which users had previously subtweeted. Figure \ref{fig:known_subtweeters} shows the distributions of subtweet probabilities produced by the classifier on all these accounts.
...
\begin{figure}[H]
	...
	\\
	$(\mu=0.520, \sigma=0.127)$}
	\label{fig:known_subtweeters}
\end{figure}

\noindent
Of these 11 accounts, the Naive Bayes classifier predicts that out of 26,928 tweets, 15,538 were subtweets (i.e. the predicted subtweet probability was at least 50\%). In comparison, if we only accept classifications of at least 75\%, then just 924 tweets (3.4\%) were predicted to be subtweets. Of these accounts, we selected the users who produced the minimum percentage of subtweets (27.6\%), the median percentage of subtweets (58.8\%), and the maximum percentage of subtweets (68.4\%). Figure \ref{fig:random_subtweeters} shows the distributions of the subtweet probabilities for all the tweets from these users.
...
\begin{figure}[H]
	...
	\caption{Example of the Twitter Bot Replying to Users' Tweets}
...
We obtained 12,169 \textbf{true subtweets} and 21,411 \textbf{true non-subtweets} to be used in our ground truth dataset. After confirming they were English and removing duplicates, there were 11,288 \textbf{true subtweets} and 19,289 \textbf{true non-subtweets} remaining. In order to preserve a balance between the classes, we limited the size of \textbf{true non-subtweets} to be the same as the set of \textbf{true subtweets}. Thus, our final ground truth dataset contains 22,576 tweets. Our classifier identifies non-subtweets with an \textbf{$F_{1}$ score} of 71.66\% and subtweets with an \textbf{$F_{1}$ score} of 73.05\%. Subtweets have garnered attention from news organizations \cite{the_atlantic}, social scientists \cite{subtweets_communications}, and governmental officials \cite{the_washington_post_trump}, but sentiment analysis of subtweets has been entirely unresearched. Utilizing the Twitter API, we acquired data for training and testing a Naive Bayes classifier, and developed a Twitter bot which actively calls-out subtweets in real time.
...
\bib{atlantic_bots}{article}{
	author={Guilbeault, Douglas}, 
	author={Woolley, Samuel},
	title={How Twitter Bots Are Shaping the Election},
	date={2016},
	eprint={https://goo.gl/ez155d},
}

\bib{pentametron}{article}{
	author={Bhatnagar, Ranjit}, 
	title={Pentametron},
	date={2018},
	eprint={https://twitter.com/pentametron}
}
...
\bib{github}{article}{
	...
	title={Senior-Project-Subtweets},
	date={2018},
	eprint={https://github.com/segalgouldn/Senior-Project-Subtweets},
}
